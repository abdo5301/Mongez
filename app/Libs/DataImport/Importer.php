...
class Importer{
    ...
    public function getList($startPage = 1,$endPage = 1){
        ...
        $URL = $connector::generateURL($this->area,$this->type,$this->purpose,$this->space,$this->price);
    ...
    public function getProperties(){
        ...
        foreach ($this->list as $value){
            ...
            if($propertyDataFromConnector){
                ...
                // --- Notification
                $numRequests = Property::requestsForImport($this->importer->property_type_id,$this->importer->purpose_id,$this->importer->area_id,$propertyDataFromConnector['space'],$propertyDataFromConnector['price'])
                    ->count();
                if($numRequests){
                    ...
                    notifyStaff(
                        ...
                        __('There are :number requests related to Importer',['number'=> $numRequests]),
                        route('system.importer.show',$this->importer->id).'?id='.$insertImporterData->id
                    );
                }
                // --- Notification
